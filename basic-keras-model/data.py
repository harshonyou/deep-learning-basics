...
def process_data(data_path, file_path):
    ...
    masks = [os.path.join(
        data_path, f"annotations\\trimaps\\{name}.png") for name in names]

    return images, masks


def load_data(path):
    train_valid_path = os.path.join(path, "annotations\\trainval.txt")
    test_path = os.path.join(path, "annotations\\test.txt")
    ...
    train_x, valid_x = train_test_split(
        train_x, test_size=0.2, random_state=42)
...
if __name__ == "__main__":
    ...
    print(
        f"Dataset: Train: {len(train_x)} - Valid: {len(valid_x)} - Test: {len(test_x)}")
